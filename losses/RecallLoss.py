...
class MulticlassRecall(Loss):
    
    def __init__(self, reduction = losses_utils.ReductionV2.AUTO, name=None, nClasses : int = 2, class_weights: [float] = None):
        super(MulticlassRecall, self).__init__(reduction, name)
    ...
    def __call__(self, y_true, y_pred, sample_weight=None):
        ...
        fn = tf.reduce_sum(self.class_weights * tf.cast(y_true * (tf.constant(1.0, shape=(1, self.nClasses)) - predictedLabels), tf.float32), axis = 1) + 1.0e-10
        result = tp / (tp + fn)
