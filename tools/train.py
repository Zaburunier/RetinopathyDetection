        ...
        for epoch in range(self.epoch_start, self.kwargs["epochs"] + 1):
            ...
            for batch_idx, batch in enumerate(self.train_loader, self.batch_start + 1):
                ...
                if self.train_index % self.kwargs["vis_print_per_iter"] == 0:
                    ...
                    printer.info(
                        f"Epoch| batch : {epoch} | {batch_idx} / {len(self.train_loader)} -> Train Loss: {loss.item()}")
                ...
                if self.train_index % self.kwargs["test_per_iter"] == 0:
                    ...
                    if self.early_stopper.early_stop(val_loss):
                        printer.warning("Ранняя остановка")
                        exit("Программа остановлена ​​методом ранней остановки")
                    ...
                    if val_loss < self.best_loss and self.kwargs["logging_active"]:
                        ...
                        printer.info("Модель сохранено")
                        printer.info(f"Best Loss: {self.best_loss} | Accuracy: {val_acc}")

        print("Конец обучение :)")

    def test(self):
        printer.info("Проверка ...")
        self.model.eval()
        test_loss = []
        test_acc = []
        with torch.no_grad():
            ...
            printer.info(f"Средняя потеря при тестировании: {np.mean(test_loss)}")
            printer.info(f"Средняя точность теста:  {np.mean(test_acc)}")
