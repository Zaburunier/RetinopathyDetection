    ...
    def init_logger(self):
        if self.is_active:
            if self.logger_name == "wandb":
                ...
                raise ValueError("Неверный тип регистратора !!")
        else:
    ...
    def log_model(self, model, epoch, step, loss, acc):
        if self.is_active:
            ...
            torch.save(model.state_dict(),
                       f"{models_dir}/net_best_epoch_{epoch}__iter_{step}__loss_{round(loss, 4)}__acc_{acc}.pth")
...
class WandbLogger(ILog):
    ...
    def log_model(self):
        if self.is_active:
            raise NotImplementedError("Для сохранения модели не реализовано ведение журнала Wandb.!!")
